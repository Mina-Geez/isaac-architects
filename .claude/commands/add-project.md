...
2. Derive a kebab-case **slug**. Create `website/content/projects/<slug>/index.md` with TOML frontmatter:
   ```
   +++
   name = "…"
   location = "…"
...
3. Ask me to drop the hero image as `website/content/projects/<slug>/hero.jpg` and gallery images as `01.jpg`, `02.jpg`, … in that folder (or give paths and copy them in). Add one `[[gallery]]` entry per image.
4. Run `python website/build.py`. The build validates content and fails loudly on missing fields/images.
